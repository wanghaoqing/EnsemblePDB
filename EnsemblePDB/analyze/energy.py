...
def get_potential_from_dist(dist, nbins=300, temp=298, save_to=None):
    ...
    data['E'] = data['f_bin'].apply(lambda x: -1.38*temp*6.02/4184*np.log(x/f_random))
    ...
    if save_to:
        data.to_csv(save_to)
    # return data[['bin min', 'bin max', 'bin mid','f_bin','E']].astype('float')
    return data[['bin min', 'bin max', 'bin mid','f_bin','E']].astype('float').sort_values(by='bin mid').drop_duplicates()

def get_energy_from_stats(potential, measured, default_maximum=True, set_maximum=None):
    '''
    Arguments:
        potential (pandas dataframe): potential energy dataframe wih columns: bin min, bin max and E
        measured (float): measured geometric parameter to get energy of
        default_maximum (bool): if True, maximum eneregy will be assigned to geometric parameters not found in any bins that the potential energy function covers. If False, will use the value specified in set_maximum {default: True}
        set_maximum (float): set maximum energy value when the geometric parameter is not found in any bins that the potential energy function covers.
    Returns
    ...
    if len(dfE) == 0:
        if default_maximum:
            ...
            return set_maximum
        else: assert("Need to specify set_maximum if not using default maximum energy")
    else:
        return dfE.iloc[0]['E']
