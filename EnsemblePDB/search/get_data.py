...
minimum_info = {
    ...
    'refine.ls_R_factor_R_work':'refine: Rwork', 
    'refine.ls_R_factor_R_free': 'refine: Rfree',
    'refine.ls_R_factor_all': 'refine: R all',
    'refine.ls_d_res_high': 'refine: resolution high',
    'refine.ls_d_res_low': 'refine: resolution low',
...
def get_pdb_data(ensemble_csv,desired_cols=minimum_info,label='MyProtein', info_list=["entry","entity","chem"], cols_to_group=[],batches=500,output_dir = None):
    ...
    if "chem" in info_list or "chem: drugbank" in info_list:
        ...
        if 'pdbx_entity_nonpoly.comp_id' not in desired_cols:
            print("'pdbx_entity_nonpoly.comp_id' needed to obtain entity information adding to desired columns")
    ...
    info_not_needed = ["entry","entry: pubmed","entity","entity: uniprot","instance","assembly","chem","chem: drugbank"]
    ...
    for i in tqdm(range(len(pdb_ids)), total=len(pdb_ids), desc='Adding data'):
        ...
        if "entry: pubmed" not in info_not_needed:
            pubmed_info = get_pubmed_entry_data(pdb,desired_cols, cols_to_group)
        ...
        if "entity" not in info_not_needed or "entity: uniprot" not in info_not_needed:
            ...
            entities = table_utils.get_list_from_table_entry(info['rcsb_entry_container_identifiers.entity_ids'])
            for entity in entities:
                if "entity" not in info_not_needed:
                    entity_info = get_pdb_entity_data(pdb,entity,desired_cols, cols_to_group)
                    info = combine_dict_info(info,entity_info,old_info)
                if "entity: uniprot" not in info_not_needed:
                    uniprot_info = get_uniprot_entity_data(pdb,entity,desired_cols, cols_to_group)
                    info = combine_dict_info(info,uniprot_info,old_info)

        if "instance" not in info_not_needed:
            old_info = list(info.keys())
            num_chains = int(info['rcsb_entry_info.deposited_nonpolymer_entity_instance_count'])+int(info['rcsb_entry_info.deposited_polymer_entity_instance_count'])
            ...
            for chain in chains:
                chain_info = get_pdb_instance_data(pdb,chain,desired_cols, cols_to_group)
                info = combine_dict_info(info,chain_info,old_info)

        if "assembly" not in info_not_needed:
            old_info = list(info.keys())
            assemblies = table_utils.get_list_from_table_entry(info['rcsb_entry_container_identifiers.assembly_ids'])
            for assembly in assemblies:
                assembly_info = get_assembly_data(pdb,assembly,desired_cols, cols_to_group)
        ...
        if ("chem" not in info_not_needed or "chem: drugbank" not in info_not_needed) and\
            ...
            if int(info['rcsb_entry_info.nonpolymer_entity_count']) > 0:
                chems = table_utils.get_list_from_table_entry(info['pdbx_entity_nonpoly.comp_id'])
                for chem in chems:
                    if "chem" not in info_not_needed:
                        chem_info = get_chemcomp_data(chem,desired_cols, cols_to_group)
...
def get_pdb_entity_data(pdb, entity,desired_cols,cols_to_group=[]):
    ...
    alt_urls = ['https://data.rcsb.org/rest/v1/core/nonpolymer_entity/','https://data.rcsb.org/rest/v1/core/branched_entity/']
...
def get_pdb_instance_data(pdb,chain,desired_cols,cols_to_group=[]):
    ...
    alt_urls = ['https://data.rcsb.org/rest/v1/core/nonpolymer_entity_instance/','https://data.rcsb.org/rest/v1/core/branched_entity_instance/']
...
def parse_dict_info(info,desired_cols,prefix="",cols_to_group=[], collapsed_dict=None, check_group=None):
    ...
    if len(cols_to_group)>0:
        ...
        for group in range(0,len(cols_to_group)):
            col_group[group] = ''
            shortest_col[group] = [col for col in cols_to_group[group] if len(col)==min([len(x) for x in cols_to_group[group]])][0]
    ...
    if type(info)==dict:
        ...
        for item in info:
            temp = parse_dict_info(item,desired_cols,prefix,cols_to_group,collapsed_dict,check_group)
            new_collapsed_dict = combine_dict_info(new_collapsed_dict,temp,old_info)
    ...
    for key,value in temp.items():
        if key not in old_info:
            if key not in new_collapsed_dict:
                ...
                new_collapsed_dict[key] = table_utils.delete_all_one_value(new_collapsed_dict[key]+" ~ "+str(value))
    return new_collapsed_dict
